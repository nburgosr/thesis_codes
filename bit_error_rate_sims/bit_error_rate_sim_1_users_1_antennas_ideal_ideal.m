...
imports
%Simulación para 1 usuario y 1 antena, estimación de canal sin reflexiones
%ni obstáculos en un canal de espacio libre.
...
messageTitle = sprintf('BER%dX%d ideal ideal',nUsers,nAntennas);
title = 'ber_sim_1X1_ideal_ideal';
...
try
    for k = 1:1:length(snr)    
        ...
        for iteration = 1:1:iterationsNum
            allBits = baseBandBERWithSeed(nUsers,totalNBits+nBitsIteration,bitsSeed);  % Generacion de una secuencia aleatoria para cada usuario
            tic
            counter = 0;
            ...
            parfor j = 1:1:totalNBits/nBitsIteration
                ...
                previousBits = allBits(:,(j-1)*nBitsIteration+1:nBitsIteration*(j)); % Generacion de una secuencia aleatoria para cada usuario
                ...
                signalEstimations = signalEstimationWithNoise(b,hhat,nAntennas,nUsers,snr(k));
