...
%ni obstáculos en un canal donde hay reflexiones y obstáculos.
...
title = sprintf("ber_sim_%dX%d_ideal_refl",nUsers,nAntennas);
...
try
    for k = 1:1:length(snr)    
        ...
        for iteration = 1:1:iterationsNum
            ...
            for j = 1:1:totalNBits/nBitsIteration
                ...
                signalEstimations = signalEstimationWithNoise(b,hrhat,nAntennas,nUsers,snr(k));
    ...
    BERTh=(1/2)*erfc(sqrt(SNR/(1+1-alfa)));                                                % Calculo del BER teorico
