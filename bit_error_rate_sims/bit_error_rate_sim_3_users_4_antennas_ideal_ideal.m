...
imports
%Simulación para 3 usuario y 4 antenas, estimación de canal sin reflexiones
...
iterationsNum = 5;                                                          % Nï¿½mero de repeticiones de cada iteration por valor de SNR
...
axy = [0 3; 0 0; 3 3; 3 0];
pxy = [2.25 0.75;1.5 sqrt(2*0.75^2)+1.5;0.75 0.75];
...
try
    for k = 1:1:length(snr)    
        ...
        for iteration = 1:1:iterationsNum
            ...
            parfor j = 1:1:totalNBits/nBitsIteration
                ...
                signalEstimations = signalEstimationWithNoise(b,hhat,nAntennas,nUsers,snr_iteration);
    ...
    BERTh=(1/2)*erfc(sqrt(SNR/(2-alfa)));                                                % Calculo del BER teorico
